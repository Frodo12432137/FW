...
OUTPUT_DIR = r"D:\app\weatherCorrection_fv\Kierunek\artifacts"
...
if USE_15MIN:
    ...
    VALID_MINUTES = [0]
    ROLLING_FEATURES = [
        ("kierunekWiatru", "kierunek_srednia4", 4),
        ("predkoscWiatru", "predkosc_srednia12", 12),
...
def load_and_prepare(sql_prognoza: str, sql_wykonanie: str) -> pd.DataFrame:
    ...
    # diff dla kierunku (kierunek wykonanie - kierunek prognoza)
    merged['diff'] = merged["kierunekWiatruLokalizacja_wykonanie"] - merged["kierunekWiatru"]

    # Dodajemy lagi diff dla kierunku
    # dla 15-min jednostek: 1h..6h = 4..24 próbek
    if USE_15MIN:
        LAGS = list(range(4, 300))  # 4..24
    else:
        # dla godzinowych, jeżeli kiedyś użyte: 1..24 godzin
    ...
    # przygotuj listę kolumn rolling do wymuszenia w dropna
    roll_names = [r for _, r, _ in ROLLING_FEATURES]

    # teraz dropna: uwzględniamy, że lagi mogą wprowadzać NaN; jeśli chcesz wymusić pełne lagi, dodaj je tutaj
    # obecnie nie dodajemy lagów do dropna, żeby nie tracić całego wczesnego okna; jeśli chcesz wymusić, dopisz *lag_names
    lag_names = [f"diff_lag_{l}" for l in LAGS]
    ...
    # upewnij się że godzina, miesiac i diff pozostają (ponowne przypisanie po get_dummies)
    merged['godzina'] = merged['dataGodzinaCET'].dt.hour
    merged['miesiac'] = merged['dataGodzinaCET'].dt.month
    merged['diff'] = merged["kierunekWiatruLokalizacja_wykonanie"] - merged["kierunekWiatru"]
...
def train_model(df: pd.DataFrame, outdir: Path, random_state: int = 42, epochs: int = 1000,
                lr: float = 0.009, weight_decay: float = 0.0001, test_size: float = 0.2):
    # 1) Zestaw cech
    roll_names = [r for _, r, _ in ROLLING_FEATURES]
